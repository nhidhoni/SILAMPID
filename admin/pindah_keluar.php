<?php
// File: admin/pindah_keluar.php - Form Pindah Keluar (Auto-fill + catatan_admin kosong)
...
function generateNoSurat($conn) {
    $tahun = date('Y');
    $bulan = date('m');
    $query = "SELECT COUNT(*) as total FROM pengajuan WHERE jenis_surat='Pindah Keluar' AND YEAR(tgl_pengajuan)='$tahun'";
    $result = mysqli_query($conn, $query);
    $count = mysqli_fetch_assoc($result)['total'];
    $urutan = str_pad($count + 1, 4, '0', STR_PAD_LEFT);
    return "474.7/$tahun/$bulan/$urutan";
}
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $tanggal_keluar = isset($_POST['tanggal_keluar']) ? mysqli_real_escape_string($conn, $_POST['tanggal_keluar']) : '';
    $status_kk = isset($_POST['status_kk']) ? mysqli_real_escape_string($conn, $_POST['status_kk']) : '';
    ...
    $query = "INSERT INTO pengajuan (id_penduduk, nik_warga, nama_warga, jenis_surat, tgl_pengajuan, status, kode_unik, catatan_admin) 
              VALUES (0, '$nik', '$nama', 'Pindah Keluar', CURDATE(), 'Pending', '$kode_unik', '')";
...
<head>
    ...
    <title>Formulir Pindah Keluar - SILAMPID</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-submit { background: #f39c12; color: white; padding: 12px 25px; border: none; border-radius: 8px; cursor: pointer; font-weight: bold; }
...
<body>
    <div class="container">
        <h1><i class="fas fa-sign-out-alt"></i> Formulir Pindah Keluar</h1>
        <div class="subtitle">Form pendaftaran pindah keluar</div>
        
        <?php if($error): ?>
            <div class="alert-error"><i class="fas fa-exclamation-triangle"></i> <?php echo $error; ?></div>
        <?php endif; ?>
        
        <form method="POST" id="pindahKeluarForm">
            <!-- Data Pemohon (dengan auto-fill) -->
            <div class="form-section">
                <h3><i class="fas fa-user"></i> Data Pemohon</h3>
                <div class="form-grid">
                    <div class="form-group">
                        <label>NIK *</label>
                        <input type="text" id="nik_pemohon" name="nik" maxlength="16" pattern="[0-9]{16}" required placeholder="16 digit NIK">
                    ...
                    <div class="form-group full-width"><label>Alamat Asal *</label><textarea id="alamat_asal" name="alamat_asal" rows="2" required placeholder="Alamat lengkap sebelum pindah" readonly></textarea></div>
                </div>
            </div>
            ...
            <div class="form-section">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group"><label>Tanggal Keluar *</label><input type="date" name="tanggal_keluar" required></div>
                    <div class="form-group"><label>Status Kartu Keluarga</label>
                        <select name="status_kk">
                            <option value="">Pilih</option>
                            <option value="Pindah Semua">Pindah Semua Anggota</option>
                            <option value="Pindah Sebagian">Pindah Sebagian Anggota</option>
                            <option value="Pisah KK">Pisah Kartu Keluarga</option>
                        </select>
                    </div>
                </div>
            </div>
    ...
    <div id="successModal" class="alert-overlay">
        <div class="alert-box">
            ...
            <p>Data pindah keluar telah berhasil disimpan.</p>
            <div class="kode-unik">
    ...
    <script>
        // Auto-fill untuk NIK Pemohon
        document.getElementById('nik_pemohon').addEventListener('blur', function() {
                ...
                fetch('ajax_get_penduduk.php?nik=' + encodeURIComponent(nik))
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            document.getElementById('alamat_asal').value = data.alamat || '';
                            ...
                            document.getElementById('alamat_asal').value = '';
